...
destination_folder = "F:/ESCOM-activo/Trabajo terminal/Extractor/CombinedData/CombinedRaceData"
...
for year in os.listdir(source_folder):
    ...
    for grand_prix in os.listdir(year_path):
        ...
        for driver in os.listdir(grand_prix_path):
            ...
            # Ignorar carpetas FP1, FP2, FP3 y Qualifying
            ignore_folders = {"FP1", "FP2", "FP3", "Qualifying"}
            if driver in ignore_folders:
                continue
            
            for file in os.listdir(driver_path):
                if file.endswith(".csv"):
                    file_type = file.split('_')[-1].replace('.csv', '')
                    identifier = f"{grand_prix}_{year}_{circuit_name}_{driver}_Race_{file_type}"
                    new_filename = f"{identifier}.csv"
                    new_filepath = os.path.join(destination_folder, new_filename)
                    
                    df = pd.read_csv(os.path.join(driver_path, file))
...
print("Race data processing complete.")
