...
# Función principal para extraer telemetría
for year in TARGET_YEARS:
    ...
    try:
        ...
        for _, event in schedule.iterrows():
            ...
            try:
                print(f"Cargando sesión: Race para {circuit} ({year})...")
                session = ff1.get_session(year, event['EventName'], "Race")
                session.load()
                print("Sesión de carrera cargada")

                # Obtener los tres primeros del podio
                results = session.results
                top_3 = results.iloc[:3]
                ...
                for _, driver in top_3.iterrows():
                    ...
                    base_folder = f"output/{year}/{circuit}/{driver_name}"
                    ...
                    # Filtrar datos para el piloto específico
                ...
                print(f"Error procesando la carrera en {circuit} ({year}): {e}")
